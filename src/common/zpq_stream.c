...
typedef struct
{
	 ...
	 * context passed to the function rx_data: received data (compressed data
	...
	/*
	 * Write up to "size" raw (decompressed) bytes. Returns number of written
...
typedef struct ZstdStream
{
	...
	size_t		tx_buffered;	/* Data which is consumed by ztd_write but not
								 * yet sent */
...
zstd_write(ZpqStream * zstream, void const *buf, size_t size, size_t *processed)
{
	...
	do
	{
		...
		{
			zs->tx.dst = zs->tx_buf;	/* Reset pointer to the beginning of
										 * buffer */

			if (in_buf.pos < size)	/* Has something to compress in input
									 * buffer */
				ZSTD_compressStream(zs->tx_stream, &zs->tx, &in_buf);

			if (in_buf.pos == size) /* All data is compressed: flushed
...
zlib_read(ZpqStream * zstream, void *buf, size_t size)
{
	...
	while (1)
	{
		if (zs->rx.avail_in != 0)	/* If there is some data in receiver
...
zlib_write(ZpqStream * zstream, void const *buf, size_t size, size_t *processed)
{
	...
	do
	{
		if (zs->tx.avail_out == ZLIB_BUFFER_SIZE)	/* Compress buffer is
													 * empty */
		{
			zs->tx.next_out = zs->tx_buf;	/* Reset pointer to the  beginning
											 * of buffer */

			if (zs->tx.avail_in != 0 || (zs->tx_deflate_pending > 0))	/* Has something in
																		 * input or deflate
																		 * buffer */
			{
				rc = deflate(&zs->tx, Z_SYNC_FLUSH);
				Assert(rc == Z_OK);
				deflatePending(&zs->tx, &zs->tx_deflate_pending, Z_NULL);	/* check if any data
																			 * left in deflate
																			 * buffer */
